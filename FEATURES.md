...
> **Version 1.1.1** · Dual-Pipeline Uber Shader for Unity  
...
All 6 shader variants expose **identical features, properties, and inspector layout** (833 properties) and are maintained in 100% lockstep parity.
...
- **LTCGI (Linearly Transformed Cosines) Area Lighting**: Real-time polygon area lighting evaluation with specular and diffuse response, polygon clipping, and integer-optimized loops for video screens and emissive meshes in LTCGI-compatible worlds across both BRP and URP (`_UseLTCGI`)
- **Forward+ & Clustered Lighting (URP 17+ / Unity 6)**: Native support for URP 17 / Unity 6 clustered light looping (`_CLUSTER_LIGHT_LOOP`), evaluating forward additional lights per fragment with pre-sampled hair specular optimizations
...
- **Screen-Space Refraction**: Real glass/water refraction using the camera opaque texture in URP (`_CameraOpaqueTexture`) with `_GrabTexture` fallback in BRP, supporting chromatic dispersion and adjustable IOR
...
- **Safe Optional VRChat SDK Integration**: Built with a dedicated assembly definition (`FireRatShader.AvatarSetup.Editor.asmdef`) and fallback window (`FireRatAvatarSetupFallback.cs`) ensuring clean zero-error compilation in standard non-VRChat Unity projects
